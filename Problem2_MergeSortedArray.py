...
# Time Complexity: 
#-----------------
# O(M+N): M+N elements have to be traversed in worst case. 
...
class Solution:
    def merge(self, nums1: List[int], m: int, nums2: List[int], n: int) -> None:
        ...
        while n_right >= 0 and m_right>=0:
            if nums1[m_right] > nums2[n_right]:
                nums1[m_n_right] = nums1[m_right]
                m_right -= 1
            else:
                nums[m_n_right] = nums2[n_right]
                n_right -= 1
            m_n_right -= 1

        while n_right >=0:
            nums1[m_n_right] = nums2[n_right]
            m_n_right -= 1
            n_right -=1
